#!/usr/bin/env bash
if [ -z "$MY_DOMAIN" ] || [ -z "$FORWARD_TO" ] || [ -z "$FORWARDED_DOMAINS" ]
then echo "\$MY_DOMAIN, \$FORWARD_TO and \$FORWARDED_DOMAINS are required but one or more are empty or unset"; exit 1
fi
